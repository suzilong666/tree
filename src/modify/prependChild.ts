...
export function prependChild(
    ...
    // 递归处理单个节点
    const prependToNode = (node: TreeNode): TreeNode => {
        // 如果已经处理过，直接返回原节点
        if (done) return node
        ...
        if (predicate(node)) {
            ...
            const childrenArray = Array.isArray(existingChildren)
                ...
                : []
            // 将新节点插入到开头
            newNodeCopy[childrenKey] = [newNode, ...childrenArray]
            return newNodeCopy
        }
        ...
        if (Array.isArray(children)) {
            ...
            const newChildren = children.map((child) => {
                const newChild = prependToNode(child)
    ...
    if (Array.isArray(tree)) {
        ...
        for (const root of tree) {
            if (done) {
                ...
                newForest.push(prependToNode(root))
            }
        }
        return newForest
    } else {
        return prependToNode(tree)
    }
}
