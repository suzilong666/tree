// modify/appendChild.test.ts
import { appendChild } from '../appendChild'
import { TreeNode } from '../../types'

describe('appendChild 追加子节点（只操作第一个匹配）', () => {
    // 原始树结构
    ...
    it('父节点没有 children 字段时应创建 children 数组并追加', () => {
    ...
    it('森林中只修改第一个匹配的父节点', () => {
        ...
        expect(newForest[1].children).toEqual([{ id: 4 }]) // 第二个匹配未被修改
    })

    it('未找到父节点时应返回原树（引用不变）', () => {
        const newTree = appendChild(originalTree, (node) => node.id === 999, {
            id: 99,
        })
    ...
    it('新节点引用应保持不变（浅拷贝）', () => {
        ...
        const inserted = (newTree as any).children[0].children[1]
        expect(inserted).toBe(newNode) // 引用相同
    })

    it('空树应返回空数组', () => {
        const newNode = { id: 1 }
        expect(appendChild([], () => true, newNode)).toEqual([])
    })
